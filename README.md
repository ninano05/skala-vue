...
## ✅ 셀프 코드 리뷰
### 1. 컴포넌트 역할 체크
- Home화면에서 사용되고 있는 뷰에서는 각 컴포넌트들이 자신의 역할을 잘 담당하고 있다.
- 또한 작은 nav를 맡고 있는 대시보드도 여러 view에서 사용되기에 컴포넌트로 구현했다.
- 하지만 이번 프로젝트 단위가 작은 만큼 about, detail, rain 화면에서는 컴포넌트로 나누기 보다는 각 뷰에 구현했다. (추후 컴포넌트 분리 작업을 진행하면 좋을 것 같다.)
### 2. 반응형 오남용 여부 확인
- 내 프로젝트는 정말 최소한으로 구현했기 때문에 과사용된 반응형은 없다.
### 3. API 요청 중 발생 상황 처리
- API 요청 중에서도 발생하는 로딩과, 에러에 대해서 try-catch로 처리하고 있다.
- 페이지 전환을 하며 발생하는 API 데이터 로딩에서는 로딩 화면을 추가하여 처리를 진행하고 있다.
- error가 발생했을 때에는 router로 Notfound로 접근하도록 하고 있다.
### 4. 변수, 함수 이름의 적절성
- 변수의 특징, 함수의 동작을 나타낼 수 있도록 적절히 설정했다.
